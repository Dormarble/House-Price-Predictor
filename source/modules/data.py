...
class Data:
    ...
    def __init__(self, complex_name, supply_area, address, recent_price, recent_contact_date, floor, sell_date):
        self.pyeong_complex_df = pd.read_csv(dirname(__file__) + '/csv/pyeong_complex.csv', index_col='idx')
        ...
        self.dist_station = float(self.complex['distStation'])
        self.dist_hospital = float(self.complex['distHospital'])
        self.dist_hangang = float(self.complex['distHangang'])
